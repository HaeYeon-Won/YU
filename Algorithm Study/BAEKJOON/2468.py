...
"""
처음에는 BFS를 함수로 사용하고 deepcopy 모듈을 이용하여 수위가 변경될 때 마다 배열을 복사하였다.
하지만 메모리 초과가 발생하였고, deepcopy를 사용하는 대신 visited를 변경시켜 값을 갱신하게 하였다.
하지만 이또한 메모리 초과가 발생하였고, 함수 호출시 발생하는 메모리 사용도 제거하였다.
그 결과 성공하였다...
"""
